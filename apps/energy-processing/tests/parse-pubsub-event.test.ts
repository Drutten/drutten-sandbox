...
describe('parsePubSubEvent', () => {
  it('decodes an EnergyImportStaged event from the Eventarc envelope', async () => {
    const delivery = await parsePubSubEvent(pubsubRequest(stagedEvent));

    assert.equal(delivery.cloudEventId, 'message-1');
    assert.equal(delivery.pubsubMessageId, 'message-1');
    assert.equal(delivery.event.eventId, 'staged-event-1');
  ...
  it('rejects when the event ID attribute and payload disagree', async () => {
    const request = pubsubRequest(stagedEvent, {eventId: 'different-event'});
...
function pubsubRequest(
  ...
  attributes: Record<string, string> = {eventId: 'staged-event-1'},
): IncomingMessage {
  const body = {
    message: {
      ...
      attributes,
      messageId: 'message-1',
      publishTime: '2026-08-09T10:00:01.000Z',
    },
  };
