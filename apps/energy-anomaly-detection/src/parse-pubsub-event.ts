...
export interface EnergyImportCompletedDelivery {
  cloudEventId: string;
  pubsubMessageId: string;
  publishTime?: string;
  attributes: Record<string, string>;
  event: EnergyImportCompletedEvent;
}
...
export async function parsePubSubEvent(
...
): Promise<EnergyImportCompletedDelivery> {
  ...
  if (
    ...
    throw new InvalidPubSubEventError(
      'Pub/Sub eventType attribute does not match the event payload',
      cloudEventId,
    );
  }
...
function parseDomainEvent(encodedData: string): EnergyImportCompletedEvent {
  let value: unknown;
  try {
    value = JSON.parse(Buffer.from(encodedData, 'base64').toString('utf8'));
  } catch (error) {
    throw new InvalidPubSubEventError(
      'Invalid EnergyImportCompleted payload',
      undefined,
      {cause: error},
    );
  }

  const event = objectValue(value, 'EnergyImportCompleted event');
  if (event.eventType !== 'EnergyImportCompleted') {
  ...
  return {
    ...
    stagedEventId: requiredString(event.stagedEventId, 'stagedEventId'),
    mergeJobId: requiredString(event.mergeJobId, 'mergeJobId'),
